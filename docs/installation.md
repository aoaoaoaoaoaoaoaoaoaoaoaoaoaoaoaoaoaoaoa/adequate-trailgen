...
Bare startup first honors a materialized project in the current directory, then the last valid project explicitly chosen by the user. With neither, Trailgen opens its Poolrooms project deck and writes nothing. The deck can browse to any project, lists materialized projects under the conventional `trailgen/` library, and creates `trailgen/starter-loop` only when the user chooses the Colorado sample. The library is resolved through `directories::UserDirs`; Linux therefore honors `XDG_DOCUMENTS_DIR` from `user-dirs.dirs` exactly, including its spelling and case. An unavailable or unwritable Documents location merely disables the sample action and conventional-library hint. Explicit `trailgen gui PATH` remains strict and portable, while `Ctrl+O` opens the project deck without sacrificing the active workbench.
