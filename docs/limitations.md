...
- Dated, recurring seasonal, recurring weekday, and daily hourly access/closure overlays are represented with `active_from`/`active_to`, `seasonal_from`/`seasonal_to`, `weekdays`/`day_of_week`, `time_from`/`time_to`, and a project planning moment. Turn restrictions, seasonal direction rules, and timed reservation systems are not represented. Edges are bidirectional by default, with one-way travel preserved only when source attributes prove it.
