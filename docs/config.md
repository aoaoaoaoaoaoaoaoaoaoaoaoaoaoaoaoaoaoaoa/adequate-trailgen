...
Distance and elevation constraints are stored in meters. CLI `generate --min-km --max-km` overrides the distance window for that run. `generate` can also override scalar difficulty, ascent/descent, road exposure, and low-confidence limits with `--min-difficulty`, `--max-difficulty`, `--min-ascent-m`, `--max-ascent-m`, `--min-descent-m`, `--max-descent-m`, `--max-road-fraction`, and `--max-low-confidence-fraction`. Confidence is a scalar in `[0,1]`; low-confidence route fraction is measured by distance over edges below `0.6`.
...
The CLI can override shape for one generation run with repeated `--shape` flags: `loop`, `figure-eight`, `out-and-back`, or `open`. It can also override repeated-edge tolerance with `--max-repeated-edge-fraction`; `--shape out-and-back --max-repeated-edge-fraction 1` is the permissive smoke-test setting. `generate --seed N` records the random seed in `routes/generated.manifest.json`; `LoopHunter` is deterministic today, but the manifest schema already preserves the seed needed by future stochastic solvers.
...
Generated route selection is name-based. After `generate`, `trailgen export <project> --route candidate-1 --format gpx|geojson|kml|kmz --output file` loads `routes/generated.routes.json` and the cached graph, then emits only the requested route. `trailgen report <project> --route candidate-1 --output file.md` renders a one-route diagnostic report; omit `--route` to render the full generated set. Reports load the effective constraints from `routes/generated.manifest.json`, so one-off CLI overrides such as `--min-km`, `--max-km`, shape flags, or elevation bounds remain visible in later selected reports.
