...
Both built-in solvers are deterministic. `trailgen generate --seed N` records `N` in `routes/generated.manifest.json` together with the app version, requested solver, concrete solver, effective config, fingerprinted source manifest, graph summary, exact edge-id route sequences, and artifacts. The trait boundary is intentionally narrow so a stochastic heuristic or MILP/CP-SAT backend can later target small subgraphs without changing ingestion, route export, reports, difficulty modeling, or the reproduction contract.
