...
Both built-in solvers are deterministic. `trailgen generate --seed N` records `N` in `routes/generated.manifest.json` together with the app version, requested solver, concrete solver, effective config, requested start coordinate, snapped start vertex/coordinate, snap distance, fingerprinted source manifest, one-run forbidden-area fingerprints/touched-edge counts, graph summary, exact edge-id route sequences, and artifacts. The same run writes `routes/generated.graph.json`, the effective graph after dated access overlays, one-run forbidden-area overlays, and one-run config overrides. The trait boundary is intentionally narrow so a stochastic heuristic or MILP/CP-SAT backend can later target small subgraphs without changing ingestion, route export, reports, difficulty modeling, or the reproduction contract.
