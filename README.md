...
Bare `trailgen` opens a project in the current directory, resumes the last valid project, or atomically creates the bundled starter at the operating system's Documents directory under `trailgen/starter-loop`. On Linux this honors `XDG_DOCUMENTS_DIR` exactly, including its spelling and case; Trailgen never assumes an uppercase `~/Documents`. The explicit `trailgen gui [PROJECT]` form works from anywhere, while `trailgen gui` uses the same resume policy. If the platform has no usable Documents directory, the workbench asks for a project root instead of writing into `$HOME`.

Trail projects remain ordinary portable directories rooted by `trailgen.toml`. Only the recent-project pointer is application state under `$XDG_STATE_HOME/trailgen`; preferences belong under `$XDG_CONFIG_HOME/trailgen`. On the first online opening, Trailgen makes a bounded z15 Protomaps PMTiles cut for the graph under `cache/basemap.pmtiles`; later launches mmap that local archive and make no basemap requests. The left inspector controls trailhead, distance, ascent/descent, difficulty, route shape, terrain fractions, solver, and search budget. Click the map to snap the trailhead to a real graph vertex, then strike **FIND TRAILS**. The atlas keeps candidate geometry at metric scale; sort the gallery by Pareto rank, distance, ascent, difficulty, or trail fraction, and click a plate for the full vector map and measured elevation/terrain/grade profile. Pass `--offline` to suppress basemap acquisition and display while retaining the measured trail vectors, search, candidates, and profiles. Set `TRAILGEN_BASEMAP_ARCHIVE` to use a prepared PMTiles archive instead of the conventional project cut.
