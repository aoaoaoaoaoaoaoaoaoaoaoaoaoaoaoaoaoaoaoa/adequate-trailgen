...
- `routes/generated.manifest.json`: app version, random seed, requested/concrete solver, effective config, fingerprinted source manifest plus coverage summary, one-run forbidden-area sources, graph summary, exact route edge sequences, and artifact list for reproducing a generation run
