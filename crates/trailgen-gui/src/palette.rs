...
mod tests {
    ...
    #[test]
    fn cycle_is_deterministic_unique_and_avoids_green() {
        let mut left = ColorCycle::new(LAW);
        let mut right = ColorCycle::new(LAW);
        let colors = (0..512)
            .map(|ordinal| {
                let color = left.color(ordinal);
                assert_eq!(color, right.color(ordinal));
                color
            })
            .collect::<Vec<_>>();
        assert_eq!(
    ...
    #[test]
    fn sparse_prefix_maximizes_the_next_dyadic_hue_gap() {
        let cycle = (0..8)
            .map(|ordinal| radical_inverse(ordinal, 2))
            .collect::<Vec<_>>();
        assert_eq!(cycle, [0.0, 0.5, 0.25, 0.75, 0.125, 0.625, 0.375, 0.875]);
    }
}
