...
The integration test builds a route-derived graph from each owned CSV trace, generates one tightly constrained exact loop, verifies the generation ledger, and requires at least 99.5% of the source points to survive within 2 m. These are deterministic, network-free solver replay fixtures; they do not prove that unconstrained search can discover the routes in Harriman's dense public graph.

Public-source coverage is tested separately because live providers are mutable and unsuitable for CI. The `trailgen coverage` diagnostic was run on 2026-07-21 against a fresh current-OSM Harriman shard: both fixtures had zero remote segments and zero disconnected transitions within a 40 m observation radius. Maximum separation was 2.74 m for South Lows and 5.23 m for West. The default OSM + USGS corpus used the same provider, normalization, conflation, and graph engine; USGS enriched the surrounding network but did not improve either owned trace. See [`docs/harriman-source-audit.md`](../../../../docs/harriman-source-audit.md) for the segment-level provenance audit and the missing support-point dogfood obligation.
